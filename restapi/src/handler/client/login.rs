...
use crate::{authentication::client::authentication::issue_jwt, cache::redis::{is_logged_in_with_username, login_user}};
...
async fn client_login(
    ...
    let is_logged_in = is_logged_in_with_username(&payload.username, &mut conn).await;
    ...
    if let Err(e) = login_user(&payload.username, &token, ttl_seconds, &mut conn).await {
        eprintln!("Error: Redis error {e}");
        return HttpResponse::InternalServerError().finish();
    }
